...
(function () {
    ...
    function enhanceQualityLabels(ui) {
        if (!ui.hasClass('r4-quality')) return;
        ui.find('.rezka4-card:not(.rezka4-back)').each(function () {
            var card = $(this), main = String(card.find('.rezka4-main').text() || '').trim();
            var n = parseInt(main, 10), label = n >= 1080 ? 'Full HD' : n >= 720 ? 'HD' : n >= 480 ? 'SD' : 'Low';
            card.find('.rezka4-meta').text(label + ' • HDREZKA');
            card.find('.rezka4-badge').text('▶');
        });
    }
    ...
    function decorateRezkaUI() {
        try {
            ...
            $('.rezka4-ui').each(function () {
                var ui = $(this), sub = String(ui.find('.rezka4-sub').first().text() || '').toLowerCase();
                ui.removeClass('r4-seasons r4-episodes r4-voices r4-quality');
                if (sub.indexOf('выберите сезон') !== -1) ui.addClass('r4-seasons');
                else if (sub.indexOf('выберите сери') !== -1 || ui.find('[data-r4-key^="episode:"]').length) ui.addClass('r4-episodes');
                else if (sub.indexOf('перевод') !== -1 || sub.indexOf('озвуч') !== -1) ui.addClass('r4-voices');
                else if (ui.find('[data-r4-key^="quality:"]').length) ui.addClass('r4-quality');
                enhanceQualityLabels(ui);
            });
        } catch (e) { console.log('REZKA4 decorate error', e); }
    }

    function hasGridLeftNeighbor(ui, focused) {
        try {
            if (!ui || !focused) return false;
            var a = focused.getBoundingClientRect(), cy = a.top + a.height / 2, nodes = ui.querySelectorAll('.selector');
            for (var i=0;i<nodes.length;i++) {
                var n=nodes[i]; if(n===focused||n.classList.contains('rezka4-back')) continue;
                var b=n.getBoundingClientRect(), by=b.top+b.height/2;
                if(Math.abs(by-cy)<Math.max(a.height,b.height)*.55 && b.right<=a.left+8) return true;
            }
        } catch(e){} return false;
    }

    function installGridLeftGuard() {
        if (window.rezka4_grid_left_guard) return; window.rezka4_grid_left_guard=true;
        document.addEventListener('keydown',function(e){
            var key=e.key||'',code=e.keyCode||e.which;if(!(key==='ArrowLeft'||code===37))return;
            var ui=document.querySelector('.rezka4-ui.r4-seasons,.rezka4-ui.r4-episodes');if(!ui)return;
            var focused=ui.querySelector('.selector.focus');if(!focused||!hasGridLeftNeighbor(ui,focused))return;
            e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();try{Navigator.move('left')}catch(x){}
        },true);
    }
