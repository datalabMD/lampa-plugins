...
function render(token){var tries=0;function step(){if(token!==window.rezka4_player_ui_polish_token)return;try{cleanupProbes();clean();var center=$('.player-panel__center').first(),play=center.find('.player-panel__playpause').first();if(!center.length||!play.length){if(tries++<40)return setTimeout(step,100);return}var prev=center.find('.rezka4-center-prev').first(),next=center.find('.rezka4-center-next').first();if(prev.length)prev.insertBefore(play);if(next.length)next.insertAfter(play)}catch(err){if(tries++<40)setTimeout(step,100)}}step()}
function install(){cleanupProbes();ensureStyle();if(!Lampa.Player||!Lampa.Player.listener)return;Lampa.Player.listener.follow('start',function(){cleanupProbes();window.rezka4_player_ui_polish_token=(window.rezka4_player_ui_polish_token||0)+1;var token=window.rezka4_player_ui_polish_token;setTimeout(function(){render(token)},140);setTimeout(function(){render(token)},450);setTimeout(function(){render(token)},900)});Lampa.Player.listener.follow('destroy',function(){window.rezka4_player_ui_polish_token=(window.rezka4_player_ui_polish_token||0)+1;clean()})}
